"""
Retail & Industry News Scraper — BANKRUPTCY EDITION
Scope: Closing articles related to Bankruptcy, Chapter 11, and Chapter 7 ONLY.

Changes from v10:
  1. CLOSING_KWS reduced to bankruptcy / Chapter 11 / Chapter 7 signals only.
  2. Mode hard-locked to "closing" — no --mode flag needed.
  3. NOISE_PATTERN FIXED: removed the block on "chapter 11", "bankruptcy",
     "liquidation" — those were previously filtered OUT by the noise pattern.
     They are now PRIMARY signals and must pass through.
  4. RELEVANCE: bankruptcy keywords weighted +5 (vs +3 for other terms).
  5. Output filenames carry a _bankruptcy suffix for easy identification.
  6. Claude extraction prompt updated for bankruptcy-specific fields
     (Event Type = Chapter 11 / Chapter 7 / Bankruptcy, Filing Date, etc.).
"""
...
TARGET_SITES = [
    ...
    # Extra sites that cover bankruptcy filings well
    "law360.com", "courthousenews.com", "bankruptcydata.com",
]

SITE_QUERY = " OR ".join([f"site:{s}" for s in TARGET_SITES[:8]])

# ─────────────────────────────────────────────────────────
# ✅ BANKRUPTCY-ONLY CLOSING KEYWORDS
#    Replaces the original broad CLOSING_KWS list entirely.
# ─────────────────────────────────────────────────────────
CLOSING_KWS = [
    # 🔴 Highest signal — specific chapter filings
    "chapter 11",
    "chapter 7",
    ...
    # 🟠 Formal process / protection terms
    "bankruptcy filing",
    ...
    # 🟡 Outcome terms directly tied to bankruptcy proceedings
    "liquidation bankruptcy",
    "bankruptcy liquidation",
    "bankruptcy closure",
    "closing due to bankruptcy",

    # 🟢 Supporting signals (still bankruptcy-adjacent)
    "debt restructuring",
    "creditor protection",
]

# Mode is hard-locked — no --mode flag required
_MODE      = "closing"
ACTIVE_KWS = CLOSING_KWS

print(f"🏷️  Mode: BANKRUPTCY CLOSING ONLY  ({len(ACTIVE_KWS)} keywords)")
...
LOCATION_ACTION_PATTERN = re.compile(
    ...
    r'expand|expanding|expansion|relocat|new site|new unit|'
    r'bankrupt|bankruptcy|chapter 11|chapter 7|liquidat|insolvenc|receivership)\b',
    re.IGNORECASE
)
...
# ─────────────────────────────────────────────────────────
# NOISE FILTER
# ⚠️  CRITICAL FIX vs v10: "chapter 11", "bankruptcy",
#     "liquidation" are NO LONGER blocked here — they are
#     the PRIMARY target and must pass through the filter.
#     Removed from noise:
#       - chapter 11 / bankruptcy / liquidation / ceases operations
#       - layoff / workforce reduction (co-occur legitimately with bankruptcy)
# ─────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────
# BANKRUPTCY TERMS — used in relevance scoring
# ─────────────────────────────────────────────────────────
BANKRUPTCY_TERMS = {
    "chapter 11", "chapter 7", "bankruptcy", "bankrupt",
    "insolvency", "insolvent", "receivership", "liquidation",
...
# ─────────────────────────────────────────────────────────
# RELEVANCE SCORING
# ⚠️  KEY CHANGE: bankruptcy terms now score +5 (highest).
#     In v10 these terms were blocked before scoring, so
#     bankruptcy articles could never reach MIN_RELEVANCE_SCORE.
# ─────────────────────────────────────────────────────────
def relevance_score(title: str, intent_kw: str, industry_terms: list) -> int:
    score = 0
    title_lower = title.lower()

    # Bankruptcy keyword hit — highest weight
    bankruptcy_hits = sum(1 for t in BANKRUPTCY_TERMS if t in title_lower)
    if bankruptcy_hits > 0:
        score += 5 + min(bankruptcy_hits - 1, 3)

    # Intent keyword match (e.g. "chapter 11")
...
MIN_RELEVANCE_SCORE = 3   # slightly higher bar — we want quality bankruptcy hits only
...
_SESSION_POOL = [_make_session(p) for p in PROXIES] if PROXIES else [_make_session()]

_thread_session_map: dict = {}
...
def _decode_scrape(google_url: str):
    try:
        resp = _get_session().get(
            google_url, timeout=10, allow_redirects=False, headers=HEADERS
        )
...
def _decode_gnd(google_url: str):
    if not HAS_GND:
        return None
    ...
    try:
        ...
        if b"<item>" not in content:
            ...
            if b"captcha" in snippet or b"unusual traffic" in snippet:
                ...
                print("  [RSS BLOCKED] Google returned HTML — possible rate limit")
            return []
...
if not all_results:
    ...
    print("   • Check manually: https://news.google.com/search?q=\"chapter+11\"+retail+when:2d")
else:
    print("📊 Sample cleaned titles:")
    for r in all_results[:5]:
        print(f"   [{r['industry']}] (score={r['relevance_score']}) {r['title']}")
    print()
...
if all_results:
    ...
    df = df.sort_values(
        ["relevance_score", "published_date"],
    ...
    # ─────────────────────────────────────────────────────
    # EXPORT  docs/news_data_bankruptcy.json
    # ─────────────────────────────────────────────────────
    ...
    news_data_path = os.path.join("docs", "news_data_bankruptcy.json")
...
• Event Type (write exactly "Chapter 11", "Chapter 7", or "Bankruptcy" based only on the article content)
• Filing / Event Date
  - Write exact date or month/year if mentioned; otherwise write exactly "Not specified"
• Status
  - Use phrasing like: "filed Chapter 11", "filed Chapter 7", "seeking bankruptcy protection",
    "in receivership", "liquidating", "closed due to bankruptcy", "restructuring under Chapter 11", etc.
...
| Store/Shop/Restaurant Name | Location or Full Address with zip code | Event Type | Filing / Event Date | Status | Short Description | Article Link |

📌 Rules
• Add one row per article in the order the articles are given
• If an article covers multiple businesses, create a separate row for each
• If an article contains ZERO relevant bankruptcy / closure information, still include a row with:
...
"""
    ...
    prompt_blocks = []
    for idx, batch in enumerate(prompt_batches, 1):
        label = (f" — Part {idx} of {len(prompt_batches)}" if len(prompt_batches) > 1 else "")
        articles_text = "\n".join(
            f"{i+1}. URL: {r['direct_link']}\n"
            f"   Title: {r['title']}\n"
            f"   Keyword matched: {r['keyword']} | Industry: {r['industry']}"
    ...
    with open("claude_prompt_bankruptcy.txt", "w", encoding="utf-8") as f:
        f.write("\n\n".join(prompt_blocks))

    print(f"📋 Saved claude_prompt_bankruptcy.txt  "
          f"({len(prompt_batches)} batch{'es' if len(prompt_batches) > 1 else ''} "
          f"× ≤{_CLAUDE_BATCH} articles)")

    # ─────────────────────────────────────────────────────
    # EXCEL + MASTER CSV EXPORT
    # ─────────────────────────────────────────────────────
    ...
    fname = DAILY_DIR / f"Retail_Bankruptcy_{NOW_UTC.strftime('%Y_%m%d_%H%M')}.xlsx"
    with pd.ExcelWriter(fname, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Bankruptcy")
        ws = writer.sheets["Bankruptcy"]
        for col_cells in ws.columns:
            max_len = max((len(str(c.value or "")) for c in col_cells), default=10)
            ws.column_dimensions[col_cells[0].column_letter].width = min(max_len + 2, 60)

    MASTER_FILE = MASTER_DIR / "daily_news_master_bankruptcy.csv"
    ...
    print(f"   Excel    : {fname}")
    print(f"\n👉 Next steps:")
    print(f"   1. Open claude_prompt_bankruptcy.txt")
    print(f"   2. Paste each block into claude.ai  (one block at a time)")
    print(f"   3. Copy Claude's full response into claude_response.txt")
